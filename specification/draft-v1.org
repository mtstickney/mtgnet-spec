...
* Wire Format
  ...
  form of [[http://cr.yp.to/proto/netstrings.txt][netstring]], hereafter called a /netstring+ string/. The format
  ...
  represented by the three-character netstring+ string
  '0:\textbackslash n'; this is the only case where the header may
  contain a leading '0'.

  As an example, the ascii string 'My very excellent mother' would be
  encoded as the netstring+ string '18:My very excellent
  mother\textbackslash n'.
