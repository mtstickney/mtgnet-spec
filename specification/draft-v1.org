...
* Typographical conventions
  ...
  escapes as used in =printf(1)=. Thus '\textbackslash n' is a one-character literal
...
* Wire Format
  ...
  form of [[http://cr.yp.to/proto/netstrings.txt][netstring]], hereafter called a /newtstring/. The format
  consists of a UTF-8 encoded header, arbitrary data, and a
  ...
  character. The terminating character is a newline ('\textbackslash
  n') character. Note that the header must not contain a
  Byte-Order-Mark (BOM).
  ...
  represented by the three-character newtstring '0:\textbackslash n';
  this is the only case where the header may contain a leading '0'.

  As an example, the ascii string 'My very excellent mother' would be
  encoded as the newtstring '18:My very excellent mother\textbackslash
  n'.
...
#<<error_handling>>
   ...
   does not contain an =Id= for an error result, where a newtstring
   sent by the client does not contain valid JSON, or where the
   ...
   a newtstring stream), or may forcibly close the connection. Closing
...
* Footnotes
[fn:2] The newtstring format has a limited ability for error
  correction, at the expense of dropping some data. One approach to
  re-synchronization of a newtstring stream is to ignore data until
